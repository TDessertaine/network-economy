...
class Firms:
    def __init__(self, z, sigma, alpha, alpha_p, beta, beta_p, omega):
        ...
        if type(z < 0) is bool:
            ...
            if (np.array([alpha, alpha_p, beta, beta_p])).any() < 0:
    ...
    def compute_optimal_quantities(targets, prices, e):
        ...
        if e.q == 0:
            if e.n == 1:
                demanded_products_labor = np.multiply(np.power(targets, 1. / e.b),
                                                    e.lamb_a)
            else:
            ...
            if e.n == 1:
                prices_net = np.multiply(np.matmul(e.lamb_a, np.power(np.concatenate(([1], prices)))), e.zeta)
            else:
