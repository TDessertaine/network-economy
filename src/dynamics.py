...
class Dynamics(object):
    ...
    def __init__(self, e, t_max, boost=None, store=None):
        self.eco = e
        self.t_max = t_max
        self.n = self.eco.n
    ...
    def time_t(self, t):
        ...
        s_vs_d = np.clip(self.supply / self.demand, None, 1)  # =1 if supply >= constraint

        self.budget[t] = self.budget_res + np.sum(self.q_exchange[1:, 0])

        self.q_exchange[0, 1:] = self.q_demand[t, 0, 1:] * np.minimum(1, self.budget[t] / np.dot(self.prices[t],
                                                                                                 self.q_demand[t, 0,
                                                                                                 1:] * s_vs_d[1:]))
        self.budget_res = self.budget[t] - np.dot(self.prices[t], self.q_exchange[0, 1:])

        # Real trades according to the supply constraint
        diag = np.diag(np.clip((self.supply[1:] - self.rho * self.q_exchange[0, 1:]) / (
                self.demand[1:] - self.rho * self.q_demand[t, 0, 1:]),
                               None, 1))

        self.q_exchange[1:, 1:] = np.matmul(self.q_demand[t, 1:, 1:],
                                            diag)
    ...
    def time_t_plus(self, t):
        ...
        np.fill_diagonal(self.stocks[t + 1], self.supply[1:] - np.sum(self.q_exchange[:, 1:], axis=0))
