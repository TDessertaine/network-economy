...
class Dynamics(object):
    def __init__(self, e, t_max, step_size=None, lda=None, nu=None, store=None):
        ...
        self.wage = np.zeros(int((t_max + 1) / self.step_s))
        self.wages = np.zeros((int((t_max + 1) / self.step_s), self.n))
    ...
    def clear_all(self, t_max=None):
        ...
        self.wage = np.zeros(int((self.t_max + 1) / self.step_s))
        self.wages = np.zeros((int((self.t_max + 1) / self.step_s), self.n))
    ...
    def exchanges_and_updates(self, t, f):
        """
        ...
        self.budget = self.savings + np.dot(self.q_exchange[t, 1:, 0], self.wages[t])

        self.q_demand[t, 0, 1:] = self.q_demand[t, 0, 1:] * (self.nu + (1 - self.nu) *
                                                             np.minimum(1, (self.savings + f * np.dot(self.q_exchange[t, 1:, 0], self.wages[t])) /
                                                                        (self.savings + f * self.labour[t])))
        ...
        self.q_exchange[t, 0, 1:] = self.q_exchange[t, 0, 1:] * np.minimum(1, (self.savings + f * np.dot(self.q_exchange[t, 1:, 0], self.wages[t])) / (
        ...
        self.wage[t + 1], self.wages[t + 1] = self.eco.firms.update_wages(self.supply[0] - self.demand[0],
                                                       self.supply[0] + self.demand[0], self.gains-self.losses, self.gains+self.losses,
                                                       self.step_s)

        self.prices_non_res[t + 1] = self.eco.firms.update_prices(self.prices[t],
        ...
        if t>2:
            self.wage[t + 1] *= np.exp(np.maximum(0, 0.1 * np.mean(np.log(self.prices_non_res[t + 1] / (self.prices_non_res[t])))))

    def production(self, t, f):
        """
        ...
        # (3) Price rescaling
        self.prices[t + 1] = self.prices_non_res[t + 1] / self.wage[t + 1]
        self.budget = self.budget / self.wage[t + 1]
        self.savings = np.maximum(self.savings, 0) / self.wage[t + 1]
    ...
    def discrete_dynamics(self, f):
        ...
        self.exchanges_and_updates(1, f)
        self.production(1, f)
        # End of first time-step
        t = 2
        f_n = f
        while t < int((self.t_max + 1) / self.step_s - 1):
            #f_n = f_n * np.exp(-1 * (self.supply[0] - self.demand[0])/(self.supply[0] + self.demand[0]))
            print(f_n)
            self.planning(t)
            self.exchanges_and_updates(t, f)
            self.production(t, f)
            t += 1
