...
class Dynamics(object):
    ...
    def planning(self, t):
        ...
        if self.eco.n == 1:
            ...
            self.q_demand[t, 1, 1] = np.maximum(
                self.q_opt[1] - (self.stocks[t] - self.stocks[t] * np.eye(self.n)),
                0)
        else:
            self.q_demand[t, 1:, 0] = self.q_opt[:, 0]
            self.q_demand[t, 1:, 1:] = np.maximum(
    ...
    def exchanges_and_updates(self, t):
        ...
        if self.eco.n==1:
            self.q_prod[:, 1:] = self.q_exchange[t, 1:, 1:] + np.minimum(
                self.stocks[t] - self.stocks[t] * np.eye(self.n), self.q_opt[1])
        else:
            self.q_prod[:, 1:] = self.q_exchange[t, 1:, 1:] + np.minimum(
    ...
    def production(self, t):
        ...
        if self.eco.n==1:
            self.q_used = (self.eco.q == 0) * np.multiply(np.nanmin(np.divide(self.q_prod, self.eco.j_a), axis=1),
        ...
        if self.eco.n == 1:
            self.stocks[t + 1] = np.multiply(self.stocks[t + 1], np.exp(- self.eco.firms.sigma * self.step_s))
        else:
    ...
    def discrete_dynamics(self):
        ...
        if self.n==1:
            self.supply = np.array([self.labour[1], self.eco.firms.z * self.g0 + self.s0])
        else:
        ...
        if self.eco.n==1:
            ...
            self.q_demand[1, 1:, 1:] = np.maximum(
                self.q_opt[1] - (self.stocks[1] - self.stocks[1] * np.eye(self.n)),
                0)
        else:
            self.q_demand[1, 1:, 0] = self.q_opt[:, 0]
            self.q_demand[1, 1:, 1:] = np.maximum(
